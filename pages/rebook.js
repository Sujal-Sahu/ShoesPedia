...
export default function Home({user,logout,addcart,removecart,clearcart,cart,subtotal,products,colors,sizes}) {
...
<div className="flex flex-wrap items-center  overflow-x-auto overflow-y-hidden py-10 justify-center   bg-white text-gray-800">
	<a rel="noopener noreferrer" href="/" className="flex items-center flex-shrink-0 px-5 py-3 space-x-2 text-gray-600">
		<img src="https://1000logos.net/wp-content/uploads/2017/05/Reebok-logo.png" style={{width: "30px",height: "20px"}}alt="adidas-logo" />
		<span>Rebook</span>
	</a>
  ...
  <img src="https://1000logos.net/wp-content/uploads/2021/11/Nike-Logo.png" style={{width: "30px",height: "20px"}}alt="adidas-logo" />
  ...
  <img src="https://i.pinimg.com/originals/87/a4/91/87a49143283e71a1aaa67dc9502b7af1.png" style={{width: "30px",height: "29px"}}alt="adidas-logo" />
...
export async function getServerSideProps(context){
  ...
  let colors=[];
    for(let item of products){
        if(item.title in RebookShoes){
            if(!RebookShoes[item.title].color.includes(item.color) && item.availableQty>0){
              RebookShoes[item.title].color.push(item.color);
            }
            if(!RebookShoes[item.title].size.includes(item.size) && item.availableQty>0){
              RebookShoes[item.title].size.push(item.size);
            }
        }
        else{
          RebookShoes[item.title]=JSON.parse(JSON.stringify(item));
            if(item.availableQty>0){
              RebookShoes[item.title].color=[item.color];
              RebookShoes[item.title].size=[item.size];
            }
            else{
              RebookShoes[item.title].color=[];
              RebookShoes[item.title].size=[];
            }
        }
  ...
  return{
    props:{products:JSON.parse(JSON.stringify(RebookShoes)),colors:colors,sizes:sizes}
  }
}
