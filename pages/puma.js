...
export async function getServerSideProps(context){
  ...
  let products=await Product.find({"category":"Puma"});
  let PumaShoes={};
  let sizes=[];
  let colors=[];
    for(let item of products){
        if(item.title in PumaShoes){
            if(!PumaShoes[item.title].color.includes(item.color) && item.availableQty>0){
              PumaShoes[item.title].color.push(item.color);
            }
            if(!PumaShoes[item.title].size.includes(item.size) && item.availableQty>0){
              PumaShoes[item.title].size.push(item.size);
            }
        }
        else{
          PumaShoes[item.title]=JSON.parse(JSON.stringify(item));
            if(item.availableQty>0){
              PumaShoes[item.title].color=[item.color];
              PumaShoes[item.title].size=[item.size];
            }
            else{
              PumaShoes[item.title].color=[];
              PumaShoes[item.title].size=[];
            }
        }
  ...
  return{
    props:{products:JSON.parse(JSON.stringify(PumaShoes)),colors:colors,sizes:sizes}
  }
}