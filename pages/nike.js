...
<div className="flex flex-wrap items-center  overflow-x-auto overflow-y-hidden py-10 justify-center   bg-white text-gray-800">
	<a rel="noopener noreferrer" href="/rebook" className="flex items-center flex-shrink-0 px-5 py-3 space-x-2text-gray-600">
...
export async function getServerSideProps(context){
  ...
  let products=await Product.find({"category":"Nike"});
  let NikeShoes={};
  let sizes=[];
  let colors=[];
    for(let item of products){
        if(item.title in NikeShoes){
            if(!NikeShoes[item.title].color.includes(item.color) && item.availableQty>0){
              NikeShoes[item.title].color.push(item.color);
            }
            if(!NikeShoes[item.title].size.includes(item.size) && item.availableQty>0){
              NikeShoes[item.title].size.push(item.size);
            }
        }
        else{
          NikeShoes[item.title]=JSON.parse(JSON.stringify(item));
            if(item.availableQty>0){
              NikeShoes[item.title].color=[item.color];
              NikeShoes[item.title].size=[item.size];
            }
            else{
              NikeShoes[item.title].color=[];
              NikeShoes[item.title].size=[];
            }
        }
  ...
  return{
    props:{products:JSON.parse(JSON.stringify(NikeShoes)),colors:colors,sizes:sizes}
  }
}